...
# 2. (Optional) Install yarn v4 if you don't have it
...
yarn initial:env # This runs automatically in postinstall, so you can usually skip it.
...
yarn build:local && yarn start
# Use build:local for local start to ensure correct cookie settings
```

> **⚠️ Important:** When updating to a new version of the project (after `git pull`), always run `yarn db:migrate` to ensure your database schema is up to date.
...
> This project uses [Yarn v4](https://yarnpkg.com/) as the recommended package manager.

```bash
# If you don't have yarn v4:
corepack enable && yarn set version 4.9.2
```
...
# (The .env file is automatically created when you run yarn install.)
...
The `yarn install` command generates a `.env` file. Add your API keys there.
...
# === Database ===
# If you don't have PostgreSQL running locally, start it with: yarn docker:pg
